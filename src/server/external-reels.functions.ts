...
async function query(filter: string): Promise<ExternalReel[]> {
...
export const fetchExternalReels = createServerFn({ method: "GET" }).handler(
  async () =>
    query("approved_for_demo=eq.true&blocked=eq.false&order=display_order.asc"),
);

export const fetchExternalReelsBlocked = createServerFn({ method: "GET" }).handler(
  async () => query("approved_for_demo=eq.true&blocked=eq.true&order=id.asc"),
);
